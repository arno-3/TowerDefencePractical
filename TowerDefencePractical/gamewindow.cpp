...
//            QPixmap *grass = new QPixmap();
//            grass->load(":/GrassTile.png");
//            grass->scaled(sWidth,sHeight,Qt::KeepAspectRatio, Qt::SmoothTransformation);

            grassVector[i][j] = new QLabel(this);
//            grassVector[i][j]->setPixmap(*grass);

            grassVector[i][j]->setFixedSize(sWidth,sHeight);
            grassVector[i][j]->setStyleSheet("image: url(:/GrassTile.png);");
